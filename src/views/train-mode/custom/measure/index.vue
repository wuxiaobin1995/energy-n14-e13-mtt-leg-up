    ...
    <div class="wrapper">
      <div class="left">
        ...
        <div class="btn">
          ...
          <div>
            【比值：{{ entadRate }}:{{ keepdRate }}:{{ offcenterRate }}】
          </div>
          <el-button
            class="item"
            type="primary"
...
export default {
  ...
  data() {
    return {
      ...
      audioSrc: path.join(__static, `narrate/mandarin/Train/自定义训练.mp3`),
      ...
      keepdRate: this.$store.state.settings[0].keepdRate, // 保持比
  ...
  methods: {
    ...
    initChart() {
      ...
      const intervalUp =
        (this.relativeDistance - this.original) / (this.entadRate * 15) // 上升间隔值
      const intervalDown =
        (this.relativeDistance - this.original) / (this.offcenterRate * 15) // 下降间隔值
    ...
    handleRefresh() {
      this.$router.push({
        path: '/refresh',
        query: {
          routerName: JSON.stringify('/custom-measure'),
